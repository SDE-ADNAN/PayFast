    ...
    <button type="submit" disabled={pending} className="btn-primary" style={{ width: "100%", marginTop: "var(--space-4)" }}>
      {pending ? (
        ...
        <span style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          Secure Autologin <ArrowRight size={18} />
        </span>
      )}
    </button>
  );
    ...
    <div className="auth-container">
      ...
      >
        <div style={{ textAlign: "center", marginBottom: "var(--space-8)" }}>
          <div style={{ display: "inline-flex", padding: "12px", borderRadius: "var(--radius-full)", background: "rgba(189, 0, 255, 0.1)", marginBottom: "var(--space-4)" }}>
            <Lock size={32} color="var(--accent-primary)" />
          </div>
          <h1 className="text-gradient">Welcome back</h1>
          <p style={{ color: "var(--text-secondary)", marginTop: "var(--space-2)" }}>
            Enter your credentials to access PayFast.
          </p>
        </div>
        ...
        <form action={formAction} style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
          ...
          <div>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input type="password" name="password" placeholder="••••••••" style={{ paddingLeft: "40px" }} required />
            </div>
          </div>
